...
public class WaypointCluster : MonoBehaviour {
 ...
 //       ++currentID;
    ...
    /// <summary>
    /// called by Waypoint per SendMessage
    /// </summary>
    /// <param name="source"></param>
    void WaypointClicked(Waypoint source) {
        if (OnWaypointClickedEvent != null) {
            OnWaypointClickedEvent(source);
        }
        if (GO_to_Notify != null) GO_to_Notify.OnWaypointClicked(source);
    }
    public IWaypointNotification GO_to_Notify = null;
    public delegate void OnWaypointClicked(Waypoint source);
